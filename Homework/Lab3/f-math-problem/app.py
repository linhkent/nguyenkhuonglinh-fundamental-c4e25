...
##        self.score = 0
    ...
    def getRandomMathProblem(self):
        ...
        [self.num1, self.num2, self.sign, self.answer, self.tr] =  freakingmath.generate_quiz()
...
##                     % (self.num1, self.num2, self.answer))
    ...
    def handleCorrect(self):
        if freakingmath.check_answer(self.tr, True):
            self.increaseScore()
    ...
    def handleWrong(self):
        if freakingmath.check_answer(self.tr, False):
            self.increaseScore()
            self.correctSound.play()
        else:
            self.resetScore()
    ...
    def drawText(self, event, qp):
        ...
        # Draw score
        qp.setPen(QColor("#E0F2F1"))
        qp.setFont(QFont('Roboto', 30, QFont.Bold))
